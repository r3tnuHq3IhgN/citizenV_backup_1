<template>
  <div>
    ...
    <div class="btn btn-danger" v-show="userActive == 'lock'">
      Xã/Phường đang bị khóa không thể khai báo cho các Thôn/Bản/Tổ dân phố !
    </div>
    <div class="btn btn-success" v-show="userActive == 'active'">
      Xã/Phường được mở khai báo cho Thôn/Bản/Tổ dân phố từ {{ allowOpen }} đến
      {{ allowClose }}
    </div>
    <!-- -------------------------------- -->

    <!-- Thống kê tài khoản cấp t/x -->
    <div v-show="userActive == 'active'" style="margin-top: 20px">
      <h1>Thống kê tài khoản cấp Thôn/Bản/Tổ dân phố</h1>
      <div class="table-wrapper-scroll-y my-custom-scrollbar">
        <table class="table table-bordered table-striped mb-0">
          ...
          <tbody>
            <tr v-for="(ham, index) in hamAct" :key="index">
              <td>{{ ham.hamlet }}</td>
              <td>{{ ham.email }}</td>
              <td>
                <div
    ...
    <div class="alert alert-warning" v-show="lockHam == 2">
      Khóa thành công thời hạn khai báo của Thôn/Bản/Tổ dân phố {{ hamN }}
    </div>
    <div class="alert alert-warning" v-show="lockHam == 1">
      Thôn/Bản/Tổ dân phố {{ hamN }} đang không khai báo !
    </div>
    <div class="alert alert-danger" v-show="alrHave == true">
      Thôn/Bản/Tổ dân phố {{ hamN }} đang có thời điểm khai báo, khóa khai báo
      cũ trước khi cấp mới !
    </div>
    <div class="alert alert-success" role="alert" v-show="success == true">
      Mở thời hạn khai báo thành công cho tài khoản của Thôn/Bản/Tổ dân phố
      {{ hamN }}
    </div>
    <!-- ---------------------------- -->
    ...
    <div v-show="open == true">
      <h3>Mở khai báo cho tài khoản của Thôn/Bản/Tổ dân phố: {{ hamN }}</h3>
      <div class="row">
    ...
    <div v-if="emptyRes == true && open == false">
      <div class="alert alert-danger">
        Thôn/Bản/Tổ dân phố {{ hamN }} đang khóa khai báo !
      </div>
    </div>
    <div v-if="statusLock == true && open == false">
      <div class="alert alert-danger">
        Thôn/Bản/Tổ dân phố {{ hamN }} đang không trong thời gian khai báo (thời
        điểm hiện tại {{ this.timeN }})
      </div>
    ...
    <div v-if="comActive == true">
      <div class="alert alert-success">
        Thôn/Bản/Tổ dân phố {{ hamN }} đang mở khai báo (thời điểm hiện tại
        {{ this.timeN }})
      </div>
      <div>Thời gian bắt đầu mở : {{ activeOpen }}</div>
      <div>Thời gian đóng khai báo : {{ activeClose }}</div>
      <br />
      <div class="btn btn-primary" @click="comActive = false">
        Đóng thông báo
      </div>
    </div>
    <!-- ----------------------------- -->
...
export default {
  ...
  mounted() {
    ...
    axios
      .get("/api/gethamact/" + this.user.city_code)
      .then((response) => {
        this.hamAct = response.data;
  ...
  methods: {
    checkStatus() {
      axios
        .get("/api/checktime/" + this.ham_codeN)
        .then((response) => {
          if (response.data != "") {
            this.alrHave = true;
          } else {
                  ...
                  axios
                    ...
                    .then((response) => {
                      if (
                        ...
                        axios
                          .get("/api/updateactive/" + this.ham_codeN)
                          .then((response) => {
                        ...
                        axios
                          .get("/api/updatelock/" + this.ham_codeN)
                          .then((response) => {
                            console.log(response);
                          })
                          .catch((error) => {
    ...
    deleteTimeOpen() {
      axios
        ...
        .delete("/api/deleteopen/" + this.ham_codeN)
        .then((response) => {
          if (response.data == "no") {
            this.lockHam = 1;
          } else {
