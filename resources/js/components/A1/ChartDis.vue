...
export default {
  ...
  mounted() {
    this.open = false;
    axios
      .get("/api/getalldisact")
      .then((response) => {
        //this.district = response.data;
        // this.dis = response.data[0].district;
        for (var i = 0; i < response.data.length; i++) {
          this.key_dis.push({ name: response.data[i].district });
        }
      })
      .catch((error) => {
        console.log(error);
    ...
    bindData() {
      ...
      if (this.selected != null) {
        ...
        for (var i = 0; i < this.dis_leng; i++) {
          ...
          axios
            .get("/api/getcodeofdis/" + this.selected[i].name)
            .then((response) => {
              this.d_code = response.data[0]["d_code"];
              axios
                .get("/api/chartgendermain/" + this.d_code)
                .then((response) => {
              ...
              axios
                .get("/api/chartagemain/" + this.d_code)
                .then((response) => {
