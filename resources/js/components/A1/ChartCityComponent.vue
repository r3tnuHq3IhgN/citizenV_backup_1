    ...
    axios
      ...
      .then((response) => {
        ...
        for (var i = 0; i < response.data.length; i++) {
          this.key_city.push({ name: response.data[i].city });
        }
      })
      .catch((error) => {
        console.log(error);
  ...
  methods: {
    bindData() {
      ...
      if (this.selected != null) {
        this.city_leng = this.selected.length;

        for (var i = 0; i < this.city_leng; i++) {
          this.success=true;
          this.model.city.push(this.selected[i].name);
          axios
            .get("/api/getcodeofcity/" + this.selected[i].name)
            .then((response) => {
              this.code = response.data[0]["code"];
              axios
                .get("/api/chartgendermain/" + this.code)
                .then((response) => {
              ...
              axios
                .get("/api/chartagemain/" + this.code)
                .then((response) => {
